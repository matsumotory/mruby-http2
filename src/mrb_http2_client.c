...
static int before_frame_send_callback(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
  ...
  if (!mrb_nil_p(conn->cb_block_hash)) {
    mrb_value cb_block =
        mrb_hash_get(conn->mrb, conn->cb_block_hash, mrb_str_new_cstr(conn->mrb, "before_frame_send_callback"));
    if (!mrb_nil_p(cb_block)) {
      mrb_yield_argv(conn->mrb, cb_block, 0, NULL);
    }
  }
...
static int on_frame_send_callback(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
  ...
  case NGHTTP2_HEADERS:
    if (nghttp2_session_get_stream_user_data(session, frame->hd.stream_id)) {
      ...
      for (i = 0; i < frame->headers.nvlen; ++i) {
        mrb_hash_set(conn->mrb, req_headers, mrb_str_new(conn->mrb, (char *)nva[i].name, nva[i].namelen),
                     mrb_str_new(conn->mrb, (char *)nva[i].value, nva[i].valuelen));
      }
      mrb_hash_set(conn->mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "request_headers")),
                   req_headers);
    }
    break;
  case NGHTTP2_RST_STREAM:
    ...
    break;
  case NGHTTP2_GOAWAY:
    mrb_hash_set(conn->mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "frame_send_header_goway")),
                 mrb_true_value());
    break;
  }
  if (!mrb_nil_p(conn->cb_block_hash)) {
    mrb_value cb_block =
        mrb_hash_get(conn->mrb, conn->cb_block_hash, mrb_str_new_cstr(conn->mrb, "on_frame_send_callback"));
    if (!mrb_nil_p(cb_block)) {
      mrb_yield_argv(conn->mrb, cb_block, 0, NULL);
    }
  }
...
static int on_frame_recv_callback(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
  ...
  case NGHTTP2_GOAWAY:
    TRACER;
    mrb_hash_set(conn->mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "frame_recv_header_goway")),
                 mrb_true_value());
    break;
  }
  TRACER;
  if (!mrb_nil_p(conn->cb_block_hash)) {
    mrb_value cb_block =
        mrb_hash_get(conn->mrb, conn->cb_block_hash, mrb_str_new_lit(conn->mrb, "on_frame_recv_callback"));
    if (!mrb_nil_p(cb_block)) {
      mrb_yield_argv(conn->mrb, cb_block, 0, NULL);
    }
  }
...

{
  ...
  case NGHTTP2_HEADERS:
    ...
    if (nghttp2_session_get_stream_user_data(session, frame->hd.stream_id)) {
      ...
      mrb_hash_set(mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "response_headers")),
                   response_headers);
    }
    break;
  case NGHTTP2_GOAWAY:
    mrb_hash_set(conn->mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "on_header_goway")),
                 mrb_true_value());
    break;
  }
  if (!mrb_nil_p(conn->cb_block_hash)) {
    mrb_value cb_block =
        mrb_hash_get(conn->mrb, conn->cb_block_hash, mrb_str_new_cstr(conn->mrb, "on_header_callback"));
    if (!mrb_nil_p(cb_block)) {
      mrb_yield_argv(conn->mrb, cb_block, 0, NULL);
    }
  }
...
static int on_stream_close_callback(nghttp2_session *session, int32_t stream_id, nghttp2_error_code error_code,
                                    void *user_data)
{
  struct mrb_http2_conn_t *conn;
  struct mrb_http2_request_t *req;
  ...
  if (req) {
    ...
    if (rv != 0) {
      mrb_raisef(mrb, E_RUNTIME_ERROR, "nghttp2_session_terminate_session: %S", mrb_fixnum_value(rv));
    }
  }
  if (!mrb_nil_p(conn->cb_block_hash)) {
    mrb_value cb_block =
        mrb_hash_get(conn->mrb, conn->cb_block_hash, mrb_str_new_cstr(conn->mrb, "on_stream_close_callback"));
    if (!mrb_nil_p(cb_block)) {
      mrb_yield_argv(conn->mrb, cb_block, 0, NULL);
    }
  }
                                       ...
                                       size_t len, void *user_data)
{
  ...
  if (req) {
    ...
    mrb_hash_set(conn->mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "recieve_bytes")),
  ...
  if (!mrb_nil_p(conn->cb_block_hash)) {
    mrb_value cb_block =
        mrb_hash_get(conn->mrb, conn->cb_block_hash, mrb_str_new_cstr(conn->mrb, "on_data_chunk_recv_callback"));
    if (!mrb_nil_p(cb_block)) {
      mrb_yield_argv(conn->mrb, cb_block, 0, NULL);
    }
  }
...
{
  SSL_CTX_set_options(ssl_ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_COMPRESSION |
...
{
  int32_t stream_id;
  const nghttp2_nv nva[] = {MAKE_NV(":method", "GET"), MAKE_NV_CS(":path", req->path), MAKE_NV(":scheme", "https"),
  ...
  stream_id = nghttp2_submit_request(conn->session, NULL, nva, sizeof(nva) / sizeof(nva[0]), NULL, req);
  if (stream_id < 0) {
    mrb_raisef(mrb, E_RUNTIME_ERROR, "http2_submit_request: stream_id=(%S)", mrb_fixnum_value(stream_id));
  }

  req->stream_id = stream_id;
  mrb_hash_set(conn->mrb, conn->response, mrb_symbol_value(mrb_intern_cstr(conn->mrb, "stream_id")),
...
void mrb_http2_client_class_init(mrb_state *mrb, struct RClass *http2)
{
  ...
  mrb_define_method(mrb, client, "before_frame_send_callback", mrb_http2_set_before_frame_send_callback,
                    MRB_ARGS_REQ(1));
  ...
  mrb_define_method(mrb, client, "on_data_chunk_recv_callback", mrb_http2_set_on_data_chunk_recv_callback,
                    MRB_ARGS_REQ(1));
